...
def syncLogs(userFile, nicoFile, errorFile, audioFile):
    ...
    while userIndex < userRows or nicoIndex < nicoRows:
        if userIndex < userRows:
            # print(user.iloc[userIndex]['DateTime'][0:19])
            userDateTime = datetime.strptime(user.iloc[userIndex]['DateTime'],'%m/%d/%Y %I:%M:%S %p')
        else:
            #userDateTime = datetime.strptime(user.iloc[userRows-1]['DateTime'],'%m/%d/%y %H:%M')
            trustUserData = False
        if nicoIndex < nicoRows:
            nicoDateTime = datetime.strptime(nico.iloc[nicoIndex]['DateTime'],'%m/%d/%Y %I:%M:%S %p')
        else:
    ...
    alldata.to_csv("C:\\Users\\arb17\\Documents\\Research\\Cobi Session 2 Transcripts\\all_dialogue_april19.csv",columns = ["UserID","DateTime","SessionID","ProblemID","StepID","Owner","DialogueAct","DialogueActConfidence","Spoke","StepAnswer","ClickStep","NicoMovement","Answered","Transcript"])
...
def main():
    ...
    userFile = "C:\\Users\\arb17\\Documents\\Research\\Cobi Session 2 Transcripts\\User_State.csv"
    nicoFile = "C:\\Users\\arb17\\Documents\\Research\\Cobi Session 2 Transcripts\\Nico_State.csv"
    errorFile = ""
    audioFile = ""
